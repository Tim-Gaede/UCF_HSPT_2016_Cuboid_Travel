/*
 * If we "unfold" the rectangular prism so that all of its sides lie on the same 2D plane,
 * the shortest path from the start to the end can be found by drawing a straight line.
 * However, there may be multiple valid ways to unfold the prism. This solution brute forces
 * all possible unfoldings and takes the best distance out of all of them.
 * 
 * The brute force is simplified by only moving up or to the right and trying all possible
 * rotations for the start face. Also, the search can be stopped after 6 sides have been
 * visited because it is never optimal to visit the same side twice.
 * 
 * It's also possible the the straight line drawn from the start to the end after the prism
 * is unfolded passes outside the area covered by the unfolded faces. We can allow this
 * because there will always be a better solution that doesn't pass outside.
 */
...
public class nick
{
	...
	public static final int[][] adjacentFaces = { {1, 4, 3, 5},
												  {2, 4, 0, 5},
												  {3, 4, 1, 5},
												  {0, 4, 2, 5},
												  {1, 2, 3, 0},
	...
	public static final int[][] rotation = { {-1, 0, -1, 0, 0, 0},
											 {0, -1, 0, -1, 3, 1},
											 {-1, 0, -1, 0, 2, 2},
											 {0, -1, 0, -1, 1, 3},
											 {0, 1, 2, 3, -1, -1},
	...
	public static void bruteForce(int face, int dir, int dx, int dy, int depth)
	{
		...
		// Perform all the necessary rotations and translations to the current face
		...
		for(int i = 0; i <= 1; i++)
		{
			// Calculate the next face, its rotation, and its location relative to the origin
	...
	// Find the face that the point is on and add its modified coordinates to a new point
	...
	static class Face
	{
		...
		// Rotate the face and all points on it. This method assumes that the face
